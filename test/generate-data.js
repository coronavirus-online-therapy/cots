...
function providerCreator(env) {
  const apiName = `cots-${env}`;
  const tables = {};
  const table = async (tableName) => {
    if(!(tableName in tables)) {
      tables[tableName] = await getApiTable(apiName, tableName);
  ...
  return async (provider) => {
    const providerTable = await table('ProviderTable');
    const accessPointTable  = await table('AccessPointTable');
    ...
    for(ap of provider.accessPoints) {
      await dynamodb.put({ 
        TableName: accessPointTable,
        Item: ap,
      }).promise();
    }
  };
...
function generateProvider() {
  const owner = `test-${randomString(32)}`;
  const now = new Date().toISOString();
  const accessPoints = randomList(states, 3, true).map(state => {return {
    __typename: 'AccessPoint',
    __test: true,
    createdAt: now,
    license: randomString(8),
    updatedAt: now,
    owner,
    state,
  }});
  return {
    __typename: 'Provider',
    __test: true,
    acceptedInsurance: randomList(insurances, 3),
    active: true,
    createdAt: now,
    fullName: `${randomString(names)} ${randomString(names)}`,
    gender: randomString(genders),
    languages: randomList(languages, 2),
    liabilityPolicy: randomString(8),
    licenseType: randomString(licenseTypes),
    modalities: randomList(modalities, 3),
    owner,
    phone: '916-555-1212',
    rate: randomString(rates),
    specializations: randomList(specializations, 3),
    tosAcceptedAt: now,
    updatedAt: now,
    url: 'https://www.coronavirusonlinetherapy.com',
    accessPoints
  };
}
...
function randomString(v) {
  if(typeof v === "number") {
    return Math.random().toString(20).substr(2, v)
...
function randomList(list, num, nonEmpty) {
  const randNum = Math.floor(Math.random()*num) + (nonEmpty?1:0);
  const resp = [];
  while(resp.length < randNum) {
...
function generateProviders(num) {
  ...
  return Array(num).fill({}).map(generateProvider);
}
...
async function main() {
  ...
  generateProviders(numToGenerate).asyncForEach(providerCreator(env));
}

// CONSTANTS
const rates = [ 0, 15, 25, 50 ];
